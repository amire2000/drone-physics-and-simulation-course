...
from common.drone_control import KF, KM, MAX_RPM, MOTOR_YAW_SIGNS


def yaw_torque(motor_rpms: tuple[float, float, float, float]) -> float:
    return sum(sign * KM * rpm**2 for sign, rpm in zip(MOTOR_YAW_SIGNS, motor_rpms))


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--rpm", type=float, default=0.5 * MAX_RPM)
    args = parser.parse_args()
    equal = (args.rpm,) * 4
    yaw_step = (args.rpm * 1.1, args.rpm, args.rpm, args.rpm)
    print(f"Thrust coefficient KF: {KF:.3e} N/RPM^2 (course model)")
    print(f"Reaction-torque coefficient KM: {KM:.3e} N m/RPM^2 (course model)")
